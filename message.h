...
#define SERVER_LISTEN_PORT 9017
...
int receive_from_peer(peer_t *peer, int (*message_handler)(message_t *))
{
  ...
  do {
    ...
    //  received_count = recv(peer->socket, (char *)&peer->receiving_buffer + peer->current_receiving_byte, 1024, MSG_DONTWAIT);
   ...
   // printf("Let's try to recv() %zd bytes... ", len_to_receive);
    ...
    if (received_count < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) {
        printf("Worker is not ready right now, try again later.\n");
      }
      else {
        perror("recv() from worker error");
        return -1;
      }
    } 
    ...
    // If recv() returns 0, it means that peer gracefully shutdown. Shutdown worker.
    else if (received_count == 0) {
      printf("recv() 0 bytes. Worker gracefully shutdown.\n");
      return -1;
    }
    else if (received_count > 0) {
...
int send_to_peer(peer_t *peer)
{
  ...
  do {
    ...
    // we have read as many as possible
      ...
      printf("send()'ed 0 bytes. The worker can't accept data right now. Try again later.\n");
      break;
    }
    else if (send_count > 0) {
