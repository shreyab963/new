...
int receive_from_peer(peer_t *peer, int (*message_handler)(peer_t * peer, message_t *))
{
  ...
  do {
    //check if it has been completely received
  ...
  printf("Total bytes recieved: %zu bytes.\n", received_total);
  return 0;
}
...
int send_to_peer(peer_t *peer)
{
  ...
  do {
    ...
    // we have read as many as possible
      ...
      printf("Leader unable to accept data\n");
      break;
    }
    else if (send_count > 0) {
